### Fix variables practice
### Fix this file so that it runs. Do not add any aditional lines of code.
### Only change the variable names so the code runs.
### The output should look like the follow line without the ###
### Hello, world. There were 10 cakes, but Lex Luthor stole 2. Now there's 8. And that's sad.

greeting = "Hello, world"

cakes = 10
theft = 2
remaining = cakes - theft

answer = f"the difference between {cakes} and {theft} is {remaining}."


print(f"{greeting}. There were {cakes} cakes, but Lex Luthor stole {theft}. Now there's {remaining}. And that's sad.")
